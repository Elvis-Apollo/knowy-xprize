...
export async function ensureStore() {
  ...
  await d1.batch([
    ...
    d1.prepare("CREATE TABLE IF NOT EXISTS agent_executions (id INTEGER PRIMARY KEY AUTOINCREMENT, intelligence_id TEXT NOT NULL, topic TEXT NOT NULL, question TEXT NOT NULL, decision TEXT NOT NULL, mode TEXT NOT NULL, provider TEXT NOT NULL, model TEXT NOT NULL, evidence_count INTEGER NOT NULL, created_at TEXT NOT NULL)"),
  ]);
...
export async function resetStore() {
  ...
  await d1.batch([d1.prepare("DELETE FROM intelligence_dependencies"), d1.prepare("DELETE FROM intelligence_objects"), d1.prepare("DELETE FROM query_history"), d1.prepare("DELETE FROM agent_executions"), d1.prepare("DELETE FROM source_records")]);
...
export async function logAgentExecution(input: {
  intelligenceId: string;
  topic: string;
  question: string;
  decision: string;
  ...
  const result = await db().prepare("INSERT INTO agent_executions (intelligence_id, topic, question, decision, mode, provider, model, evidence_count, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
    .bind(input.intelligenceId, input.topic, input.question, input.decision, input.mode, input.provider, input.model, input.evidenceCount, createdAt).run();
  return { id: Number(result.meta.last_row_id), createdAt };
}

export async function listAgentExecutions(limit = 20): Promise<AgentExecution[]> {
  await ensureStore();
  const result = await db().prepare("SELECT id, intelligence_id AS intelligenceId, topic, question, decision, mode, provider, model, evidence_count AS evidenceCount, created_at AS createdAt FROM agent_executions ORDER BY id DESC LIMIT ?")
    .bind(limit).all<AgentExecution>();
  return result.results;
}
